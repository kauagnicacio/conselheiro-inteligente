...
function CadastroContent() {
  ...
  const paymentApproved = searchParams.get("approved");
  const transactionId = searchParams.get("transaction_id");
  
  // Verificar se é um redirecionamento pós-pagamento aprovado
  const isPaymentConfirmed = 
    checkoutSuccess || 
    paymentStatus === "approved" || 
    paymentApproved === "true" ||
    transactionId; // Presença de transaction_id indica pagamento processado

  // Disparar evento Purchase apenas quando pagamento confirmado
  useEffect(() => {
    if (isPaymentConfirmed) {
      // Valor padrão - ajustar conforme o plano real
      // TODO: Pegar valor real do backend ou query params
      const purchaseValue = parseFloat(searchParams.get("value") || "97.00");
  ...
  if (showSuccess) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-white dark:bg-[#0a0a0a] p-4">
